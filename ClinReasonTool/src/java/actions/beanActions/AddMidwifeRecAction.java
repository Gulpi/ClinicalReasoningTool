...
public class AddMidwifeRecAction implements AddAction, Scoreable{
	...
	public void notifyLog(Relation rel) {
		...
		if(!patIllScript.isExpScript()) new TypeAheadBean(rel.getListItemId(), Relation.TYPE_MREC).save();
	...
	public void triggerScoringAction(Beans rel, boolean isJoker) {
		new ScoringAddAction().scoreAction(((RelationMidwifeRecommendation) rel).getListItemId(), this.patIllScript, isJoker, Relation.TYPE_MREC);
