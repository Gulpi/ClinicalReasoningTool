...
import beans.relation.RelationPatho;
...
public class AddPathoAction implements AddAction, Scoreable/*, FeedbackCreator*/{
	...
	public void notifyLog(Relation rel) {
		new LogEntry(LogEntry.ADDPATHO_ACTION, patIllScript.getId(), rel.getListItemId()).save();
		if(!patIllScript.isExpScript()) new TypeAheadBean(rel.getListItemId(), Relation.TYPE_PATHO).save();
	...
	public void addRelation(/*long id, String name*/ListItem li, int x, int y, long synId, boolean isJoker){
		...
		if(patIllScript.getPatho().contains(rel)){
			createErrorMessage(IntlConfiguration.getValue("patho.duplicate"),"optional details", FacesMessage.SEVERITY_WARN);
			return;
		}
		rel.setOrder(patIllScript.getPatho().size());
	...
	public void triggerScoringAction(Beans rel, boolean isJoker) {
		new ScoringAddAction().scoreAction(((RelationPatho) rel).getListItemId(), this.patIllScript, isJoker, Relation.TYPE_PATHO);
		new ScoringListAction(this.patIllScript).scoreList(ScoreBean.TYPE_PATHO_LIST, Relation.TYPE_PATHO);

	}
	public void createErrorMessage(String summary, String details, Severity sev) {
		new ErrorMessageContainer().addErrorMessage("pathoform", summary, details, sev);		
	}
